...
{
	struct dw_mci_rtkemmc_host *priv = host->priv;
	priv->gRegTbl.emmc_mux_pad0 = mci_readl(host, muxpad0);
	priv->gRegTbl.emmc_mux_pad1 = mci_readl(host, muxpad1);
	priv->gRegTbl.emmc_pfunc_nf1 = mci_readl(host, PFUNC_NF1);
	priv->gRegTbl.emmc_pfunc_cr = mci_readl(host, PFUNC_CR);
	priv->gRegTbl.emmc_pdrive_nf1 = mci_readl(host, PDRIVE_NF1);
	priv->gRegTbl.emmc_pdrive_nf2 = mci_readl(host, PDRIVE_NF2);
	priv->gRegTbl.emmc_pdrive_nf3 = mci_readl(host, PDRIVE_NF3);
	priv->gRegTbl.emmc_pdrive_nf4 = mci_readl(host, PDRIVE_NF4);

	priv->gRegTbl.emmc_pdrive_cr0 = mci_readl(host, PDRIVE_CR0);
	priv->gRegTbl.emmc_pdrive_cr1 = mci_readl(host, PDRIVE_CR1);
	priv->gRegTbl.emmc_pdrive_sdio = mci_readl(host, PDRIVE_SDIO);
	priv->gRegTbl.emmc_pdrive_sdio1 = mci_readl(host, PDRIVE_SDIO1);

	priv->gRegTbl.emmc_ctype = mci_readl(host, CTYPE);
	priv->gRegTbl.emmc_uhsreg = mci_readl(host, UHSREG);
	priv->gRegTbl.emmc_ddr_reg = mci_readl(host, DDR_REG);
	priv->gRegTbl.emmc_card_thr_ctl = mci_readl(host, CARD_THR_CTL);
	priv->gRegTbl.emmc_clk_div = mci_readl(host, CLKDIV);
	priv->gRegTbl.emmc_ckgen_ctl = mci_readl(host, CKGEN_CTL);
	priv->gRegTbl.dqs_ctrl1 = mci_readl(host, DQS_CTRL1);
}

static void restore_registers(struct dw_mci *host, struct dw_mci_slot *slot)
{
	struct dw_mci_rtkemmc_host *priv = host->priv;
	struct mmc_host *mmc = slot->mmc;

	mci_writel(host, muxpad0, priv->gRegTbl.emmc_mux_pad0);
	mci_writel(host, muxpad1, priv->gRegTbl.emmc_mux_pad1);
	mci_writel(host, PFUNC_NF1, priv->gRegTbl.emmc_pfunc_nf1);
	mci_writel(host, PFUNC_CR, priv->gRegTbl.emmc_pfunc_cr);
	mci_writel(host, PDRIVE_NF1, priv->gRegTbl.emmc_pdrive_nf1);
	mci_writel(host, PDRIVE_NF2, priv->gRegTbl.emmc_pdrive_nf2);
	mci_writel(host, PDRIVE_NF3, priv->gRegTbl.emmc_pdrive_nf3);
	mci_writel(host, PDRIVE_NF4, priv->gRegTbl.emmc_pdrive_nf4);

	mci_writel(host, PDRIVE_CR0, priv->gRegTbl.emmc_pdrive_cr0);
	mci_writel(host, PDRIVE_CR1, priv->gRegTbl.emmc_pdrive_cr1);
	mci_writel(host, PDRIVE_SDIO, priv->gRegTbl.emmc_pdrive_sdio);
	mci_writel(host, PDRIVE_SDIO1, priv->gRegTbl.emmc_pdrive_sdio1);

	mci_writel(host, CTYPE, priv->gRegTbl.emmc_ctype);
	mci_writel(host, UHSREG, priv->gRegTbl.emmc_uhsreg);
	mci_writel(host, DDR_REG, priv->gRegTbl.emmc_ddr_reg);
	mci_writel(host, CARD_THR_CTL, priv->gRegTbl.emmc_card_thr_ctl);
	mci_writel(host, CLKDIV, priv->gRegTbl.emmc_clk_div);
	mci_writel(host, CKGEN_CTL, priv->gRegTbl.emmc_ckgen_ctl);
}
